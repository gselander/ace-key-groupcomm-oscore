...
The joining node posts the Access Token to the /authz-info endpoint at the Group Manager, according to the Token post defined in Section 3.3 of {{I-D.ietf-ace-key-groupcomm}}. If allowed by the used transport profile of ACE, the joining node may instead provide the Access Token to the Group Manager by other means, e.g. during a secure session establishment (see Section 3.3.1 of {{I-D.ietf-ace-dtls-authorize}}).
...
* 'cnonce', as defined in Section 5.1.2 of {{I-D.ietf-ace-oauth-authz}}, and including a dedicated 8-byte nonce N_C generated by the Client.

* The 'client_cred_verify' parameter, including a signature encoded as a CBOR byte string, computed by the joining node to prove possession of its own private key. The joining node computes the signature by using the same private key and countersignature algorithm it intends to use for signing messages in the OSCORE group. The signature is computed over N_S concatenated with N_C. In particular, N_C is the nonce generated by the Client and specified in the 'cnonce' parameter, while N_S is defined as follows.
