...
* The joining node and the Group Manager use an asymmetric proof-of-possession key to establish a secure communication channel. In this case, the Group Manager stores the proof-of-possession key conveyed in the Access Token as the public key of the joining node. Then, two cases can occur.

   1. The stored proof-of-possession key is consistent with the counter signature algorithm and possible associated parameters used in the OSCORE group. If the joining node is aware of that, it may not provide again its own public key to the Group Manager, which considers the stored proof-of-possession key as the public key associated to the joining node in the OSCORE group. If the joining node provides its own public key in the join request anyway, the Group Manager performs consistency checks on the public key as in {{ssec-join-resp}} and, in case of success, considers it as the public key associated to the joining node in the OSCORE group.

   2. The stored proof-of-possession key is not consistent with the counter signature algorithm and possible associated parameters used in the OSCORE group. In this case, the joining node has to provide a different consistent public key to the Group Manager in the join request (see {{ssec-join-req}}). Then, the Group Manager performs consistency checks on this latest provided public key as in {{ssec-join-resp}} and, in case of success, considers it as the public key associated to the joining node in the OSCORE group.
