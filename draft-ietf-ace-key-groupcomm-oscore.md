...
At this point in time, the joining node might not have all the necessary information concerning public keys for computing/verifying countersignatures in the  OSCORE group. In such a case, the joining node MAY ask for that information to the Group Manager. To this end, it sends the CoAP POST request to the /authz-info endpoint using the Content-Format "application/ace+cbor" defined in Section 8.14 of {{I-D.ietf-ace-oauth-authz}}, and includes also the following parameters:

* 'sign_info' defined in {{sign-info}}, encoding the CBOR simple value Null, to require information and parameters on the countersignature algorithm and on the public keys used in the OSCORE group.

* 'pub_key_enc' defined in {{pub-key-enc}}, encoding the CBOR simple value Null, to require information on the exact encoding of public keys used in the OSCORE group.
...
The payload of the 2.01 (Created) response is a CBOR map, which MUST include the 'cnonce' parameter defined in section 5.1.2 of {{I-D.ietf-ace-oauth-authz}}, and MAY include the 'sign_info' parameter as well as the 'pub_key_enc' parameter defined in {{sign-info}} and {{pub-key-enc}} of this specification, respectively.
